...
if (Test-Path $sonarrConfigPath) {
    ...
    $result.sonarr.imports = @($sHistory | Where-Object { $_.eventType -eq "downloadFolderImported" -or $_.eventType -eq "episodeFileImported" } | Select-Object -First $MaxItems -Property date,eventType,sourceTitle,@{n="series";e={$_.series.title}},@{n="episode";e={($_.episodes | ForEach-Object { "S{0:00}E{1:00} {2}" -f $_.seasonNumber,$_.episodeNumber,$_.title }) -join "; "}})
    ...
    if (-not $NoQueueDetails) {
        ...
        $result.sonarr.queue = @($sQueue.records | Select-Object -First $MaxItems -Property title,status,trackedDownloadStatus,trackedDownloadState)
    }
    } catch {
...
if (Test-Path $radarrConfigPath) {
    ...
    $result.radarr.imports = @($rHistory | Where-Object { $_.eventType -eq "downloadFolderImported" -or $_.eventType -eq "movieFileImported" } | Select-Object -First $MaxItems -Property date,eventType,sourceTitle,@{n="movie";e={$_.movie.title}},@{n="year";e={$_.movie.year}})
    ...
    if (-not $NoQueueDetails) {
        ...
        $result.radarr.queue = @($rQueue.records | Select-Object -First $MaxItems -Property title,status,trackedDownloadStatus,trackedDownloadState)
    }
    } catch {
...
try {
    ...
    if ($qbitPassword) {
        ...
        $items = @($torrents | Where-Object { $_.completion_on -ge $sinceUnix -or $_.added_on -ge $sinceUnix } | Sort-Object completion_on,added_on)
        $result.qbit.count = $items.Count
        $result.qbit.items = @($items | Select-Object -First $MaxItems -Property name,state,progress,category,save_path,@{n="added_local";e={Convert-FromUnixLocal $_.added_on}},@{n="completed_local";e={Convert-FromUnixLocal $_.completion_on}})
    } else {
